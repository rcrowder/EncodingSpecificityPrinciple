...
% of channels
result_field=zeros(nr_points,length(cfs));

for ii=1:nr_points
% 	current_start_cent=start_cent+ii*delta_cent;
	count=0;
%     current_harmonic_relation(ii)=cfs(1)/cent2fre(fre_min,current_start_cent);
...
% 		res_vals(ii)=res_vals(ii)+cur_val;
        result_field(ii,jj)=cur_val;
	end
end
