...
function handlestr=plot(current_frame,options)
...
cca=gca;
% oldunit2=get(cca,'Units');
% set(cca,'Units','Pixel');
% pos=get(cca,'Position');
% breite=pos(3);
...
if nr_channels==1
	...
	if strcmp(plotstyle,'surf')
		handle=surf(cvals,'LineStyle','none');
	else
...
if time_reversed
    set(cca,'XDir','reverse')   % turn them around, because the higher values shell end on the right
else
    set(cca,'XDir','normal')   % normale ausrichtung
end        
if is_log
    min_x_screen=round(abs((minimum_time_interval-start_time+1/sr)*sr)); % thats the first point we want to see on the screen
	if nr_channels==1
...
if has_x_axis
    if max(tix)>1
        set(cca,'XTick',tix);
        set(cca,'XTickLabel',ti);
    end
else
    set(cca,'xtick',[]); % we dont want any z-Ticks!
end % axis    
...
elseif nr_channels>1
    set(cca,'ytick',[]); % we dont want any z-Ticks!
end

set(cca,'ztick',[]); % we dont want any z-Ticks!
